...
// Regular-user authentication using Firebase Auth.
// Verification emails are generated by Firebase Admin SDK on the server and
// delivered through a domain-authenticated transactional email provider.
// This keeps the provider API key off the browser and gives the email better
// sender authentication/reputation than Firebase's default sender.
function UserAuthModal({ onClose, onSuccess }) {
  ...
  // Calls the trusted backend. The backend verifies this Firebase ID token,
  // generates the Firebase verification link with Admin SDK, and sends the
  // branded email through the configured transactional email provider.
  const sendVerification = async (user) => {
    ...
    const response = await fetch(`${API_BASE_URL}/api/send-verification`, {
      ...
      body: JSON.stringify({
        displayName: user.displayName || name.trim() || "there",
      }),
    });
    ...
    if (!response.ok) {
      const err = new Error(data.message || "Could not send verification email.");
      err.code = data.code || "verification/send-failed";
      throw err;
    }
  ...
  const handleSignIn = async () => {
    ...
    if (!user.emailVerified) {
      ...
      setInfo(
        "Your email is not verified yet. We sent a fresh verification email. " +
        "Please check your Inbox, then Spam/Junk if needed."
      );
      return;
    }
  ...
  const handleSignUp = async () => {
    ...
    try {
      // The user is authenticated at this point, so the backend can verify
      // the ID token before generating/sending the verification link.
      await sendVerification(user);
    ...
    setConfirmPassword("");
    setInfo(
      "Account created successfully! We sent a verification email to your address. " +
      "Check your Inbox first. If it is not there, check Spam/Junk and search for " +
      "\"Campus Connect\"."
    );
  };
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (err?.code === "verification/send-failed") {
        setError(
          "Your account was created, but the verification email could not be sent. " +
          "Please try the resend button after a moment."
        );
      } else {
  ...
  const resendVerificationEmail = async () => {
    ...
    try {
      ...
      setInfo(
        "Verification email sent. Check your Inbox first. " +
        "If needed, check Spam/Junk and search for \"Campus Connect\"."
      );
    } catch (err) {
      ...
      } else if (err?.code === "verification/send-failed") {
        setError(
      ...
      <form onSubmit={submit} className="p-5 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            >
              {mode === "signup"
                ? <User size={20} color={THEME.gold} />
                : <LogIn size={20} color={THEME.gold} />}
        ...
        {info && (
          <div
            className="flex items-start gap-2 text-xs mb-4 px-3 py-2.5 rounded-lg"
            style={{
        ...
        >
          ...
          <p className="text-[11px] leading-relaxed" style={{ color: THEME.inkSoft }}>
            {mode === "signup"
              ? "After creating your account, Campus Connect will send a branded verification email. Check your Inbox first."
              : "If your email is not verified, a fresh verification email will be sent. Check your Inbox first."}
          </p>
